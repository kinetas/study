...
-- buytbl을 C_buytbl로 구조 + 데이터 복사하고 num을 primary key로 설정
...
-- Set null		: PK 열의 값이 변경시 FK열의 값을 NULL로 설정
-- Set Default	: PK 열의 값이 변경시 FK 열의 값은  Default 로 설정된 기본값을 적용

create table tbl_test2(
	no int primary key,
    id varchar(45) not null,
    constraint FK_tbl_test2_tbl_b foreign key(id) references tbl_b(id)
    on update cascade
    on delete cascade
);
desc tbl_test2;
...
drop table tbl_e; -- x 참조되는 테이블이기 떄문에 삭제 불가
...
-- num를 PK 설정
alter table copy_buytbl add constraint PK_num primary key(num);
-- userid를 FK 설정(on delete Restrict on update cascade)
alter table copy_buytbl 
add constraint FK_copy_buytbl_usertbl
foreign key(userid)
